...
void CPreviewImageDlg::UpdateUI()
{
	...
	GetDlgItem(IDC_CHK_MULTI_SYNC)->EnableWindow(IsDevicePid(ETronDI_PID_8053) || IsDevicePid(ETronDI_PID_8059) || IsDevicePid(ETronDI_PID_8062));
...
void CPreviewImageDlg::ProcessImgCallback(EtronDIImageType::Value imgType, int imgId, std::vector<unsigned char> imgBuf, int imgSize,
	int width, int height, int serialNumber)
{
	...
	if (EtronDIImageType::IsImageColor(imgType))
	{
		...
		if (imgId == ETron_Stream_Color)
		{
			...
			if (pDlg != nullptr)
			{
				pDlg->ApplyImage(&imgBuf[0], &imgSize, imgType == EtronDIImageType::COLOR_RGB24, imgType == EtronDIImageType::COLOR_MJPG, serialNumber);
#ifndef ESPDI_EG
		...
		else if (imgId == ETron_Stream_Track)
		{
			...
			if (pTrackDlg)
			{
				pTrackDlg->ApplyImage(&imgBuf[0], &imgSize, imgType == EtronDIImageType::COLOR_RGB24, imgType == EtronDIImageType::COLOR_MJPG, serialNumber);
			}
		}
		else if (imgId == ETron_Stream_Kolor)
		{
			...
			if (pKcolorDlg)
			{
				pKcolorDlg->ApplyImage(&imgBuf[0], &imgSize, imgType == EtronDIImageType::COLOR_RGB24, imgType == EtronDIImageType::COLOR_MJPG, serialNumber);
#ifndef ESPDI_EG
...
DWORD CPreviewImageDlg::Thread_Preview( void* pvoid )
{
	...
	if (FrameSyncManager::GetInstance()->IsEnable())
	{
		...
		FrameSyncManager::GetInstance()->SetIsInterleave(pThis->m_hEtronDI, pThis->m_DevSelInfo, pThis->m_bIsInterLeaveMode);
	}
...
void CPreviewImageDlg::CloseDeviceAndStopPreview(CDialog* pCallerDlg)
{
	...
	if (FrameSyncManager::GetInstance()->IsEnable())
	{
		FrameSyncManager::GetInstance()->UnregisterDevice(m_hEtronDI, m_DevSelInfo);
	}
...
{
#if 0 // test EtronDI_GetColorImage & EtronDI_GetDepthImage
	...
	if (ETronDI_OK == EtronDI_GetColorImage(m_hEtronDI, &m_DevSelInfo, &colorBuffer[0], &nColorImageSize, &nColorSerialNumber))
	{
		
		std::vector<BYTE> m_vecRGBImageBuf(nColorWidth * nColorHeight * 3, 0);

		if (ETronDI_OK != EtronDI_ColorFormat_to_RGB24(m_hEtronDI, &m_DevSelInfo,
	...
	if (ETronDI_OK == EtronDI_GetDepthImage(m_hEtronDI, &m_DevSelInfo, &depthBuffer[0], &nDepthImageSize, &nDepthSerialNumber))
	{
		std::ostringstream FileName_Depth;
		FileName_Depth << GetCurrentModuleFolder().c_str() << "\\Depth_TEST.yuv";
		if (depthBuffer.empty() == false && nDepthWidth > 0 && nDepthHeight > 0)
		{
			SaveDepthYuv(depthBuffer, EtronDIImageType::DEPTH_11BITS, nDepthWidth, nDepthHeight, FileName_Depth.str().c_str());
		}
	}
#endif
