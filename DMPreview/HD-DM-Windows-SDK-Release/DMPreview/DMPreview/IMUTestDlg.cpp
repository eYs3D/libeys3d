...
IMUTestDlg::IMUTestDlg(const wchar_t* SN, void*& hEtronDI, DEVSELINFO& devSelInfo, ModeConfig::IMU_TYPE imu_type, CWnd* pParent, CPreviewImageDlg* pPreviewDlg)
	: CDialog(IDD_DIALOG_IMU, pParent), m_hEtronDI(hEtronDI), m_DevSelInfo(devSelInfo), m_IMU_TYPE(imu_type), m_bIsSyncIMU(FALSE), m_pPreviewDlg(pPreviewDlg)
{
...
void IMUTestDlg::OnBnClickedCheckFrameSync()
{
	...
	if (FrameSyncManager::GetInstance()->IsEnable())
	{
		...
		FrameSyncManager::GetInstance()->SetIsInterleave(m_hEtronDI, m_DevSelInfo, m_pPreviewDlg->IsInterLeaveMode());
	}
	else
	{
