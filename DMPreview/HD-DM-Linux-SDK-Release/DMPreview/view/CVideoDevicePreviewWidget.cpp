...
{
    bool bPointColudAvaliable = m_pVideoDeviceController->GetPreviewOptions()->IsStreamEnable(CVideoDeviceModel::STREAM_COLOR) ||
                                m_pVideoDeviceController->GetPreviewOptions()->IsStreamEnable(CVideoDeviceModel::STREAM_KOLOR) ;
    bPointColudAvaliable = bPointColudAvaliable & m_pVideoDeviceController->GetPreviewOptions()->IsStreamEnable(CVideoDeviceModel::STREAM_DEPTH);
    ...
    switch (format){
        case PreviewOptions::POINT_CLOUDE_VIEWER_OUTPUT_COLOR:
            ui->comboBox_point_cloud_viewer_format->setCurrentText("Color"); break;
