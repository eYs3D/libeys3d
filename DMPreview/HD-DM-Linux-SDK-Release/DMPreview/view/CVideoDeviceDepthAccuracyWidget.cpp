...
void CVideoDeviceDepthAccuracyWidget::UpdateSelf()
{
    ...
    ui->groupBox_adjusting_focal_length->setEnabled(bEnable);
...
void CVideoDeviceDepthAccuracyWidget::UpdateAdjustingFocalLength()
{
    UpdatePixelUnit();
    UpdateFocalLength();
}

void CVideoDeviceDepthAccuracyWidget::UpdatePixelUnit()
{
    ui->horizontalSlider_pixel_unit_value->blockSignals(true);
    ui->horizontalSlider_pixel_unit_value->setValue(-m_pDepthAccuracyController->GetPixelUnit());
    ui->horizontalSlider_pixel_unit_value->blockSignals(false);

    ui->lineEdit_pixel_unit_vaule->setText(QString::number(-ui->horizontalSlider_pixel_unit_value->value()));
}

void CVideoDeviceDepthAccuracyWidget::UpdateFocalLength()
{
    CDepthAccuracyController::FocalLength focalLength = m_pDepthAccuracyController->GetFocalLength();

    ui->lineEdit_focal_length_left_fx_value->setText(QString::number(focalLength.nLeftFx));
    ui->lineEdit_focal_length_left_fy_value->setText(QString::number(focalLength.nLeftFy));
    ui->lineEdit_focal_length_right_fx_value->setText(QString::number(focalLength.nRightFx));
    ui->lineEdit_focal_length_right_fy_value->setText(QString::number(focalLength.nRightFy));
}

void CVideoDeviceDepthAccuracyWidget::on_horizontalSlider_pixel_unit_value_valueChanged(int value)
{
    ui->lineEdit_pixel_unit_vaule->setText(QString::number(-value));
}

void CVideoDeviceDepthAccuracyWidget::on_pushButton_clicked()
{
    m_pDepthAccuracyController->SetPixelUnit(-ui->horizontalSlider_pixel_unit_value->value());
    UpdateFocalLength();
}
