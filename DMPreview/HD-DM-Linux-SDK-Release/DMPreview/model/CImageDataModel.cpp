...
int CImageDataModel_Color::TransformRawToRGB()
{
    ...
    switch (GetImageType()){
        case EtronDIImageType::COLOR_MJPG:
            ret = MJPEG_Covert_TruboJPG(&m_rawData[0],
                                        m_nDataSize,
                                        &m_rgbData[0],
                                        m_nWidth,
                                        m_nHeight);
...
int CImageDataModel_Color::MJPEG_Covert_TruboJPG(BYTE *pMJPEG, int nMJPEGSize, unsigned char *pRGB, unsigned int nWidth, unsigned int nHeight)
{
    int ret = tjDecompress2(m_pTjHandle,
                            pMJPEG,
                            nMJPEGSize,
                            pRGB,
                            nWidth,
                            ...
                            nHeight,
                            TJPF_RGB,
                            TJFLAG_FASTDCT | TJFLAG_FASTUPSAMPLE) ? ETronDI_VERIFY_DATA_FAIL : ETronDI_OK;
    return ret;
}
