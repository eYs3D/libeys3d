...
void Motions::OnImuDataCallback(const ImuDataPacket& packet) {
  ...
  imu->accel[0] = packet._accelX;
  imu->accel[1] = packet._accelY;
  imu->accel[2] = packet._accelZ;
  imu->gyro[0] = packet._gyroScopeX;
  imu->gyro[1] = packet._gyroScopeY;
  imu->gyro[2] = packet._gyroScopeZ;
  imu->quaternion[0] = packet._quaternion[0];
  imu->quaternion[1] = packet._quaternion[1];
  imu->quaternion[2] = packet._quaternion[2];
  imu->quaternion[3] = packet._quaternion[3];
  imu->timestamp = packet.timestamp;
  imu->flag = EYS3D_IMU_ACCEL_GYRO_CALIB;
#else
