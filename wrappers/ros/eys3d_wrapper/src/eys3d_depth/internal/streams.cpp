...
#include "eys3d_depth/internal/streams.h"
...
#include "eys3d_depth/internal/match.h"
...
void Streams::DoImageDepthCaptured(const Image::pointer& depth,
  ...
  if (DepthMode::DEPTH_COLORFUL == device_->GetOpenParams().depth_mode) {
    Image::pointer depth_rgb = device_->GetColorizer()->Process(depth, DepthMode::DEPTH_COLORFUL, false);
    DoStreamDataCaptured(depth_rgb, info);
  } else if (DepthMode::DEPTH_GRAY == device_->GetOpenParams().depth_mode) {
    Image::pointer depth_gray = device_->GetColorizer()->Process(depth, DepthMode::DEPTH_GRAY, false);
    DoStreamDataCaptured(depth_gray, info);
  } else {
