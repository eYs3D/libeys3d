...
#include "eys3d_depth/data/channels.h"
...
#include "eys3d_depth/data/hid.h"
...
bool Channels::ImuCalibration()
{
  
  // Start calbration
  {
    LOGI("Start IMU Calbration.");
    SetFeatureDATA_Item setFeatureData = {&START_CALIBRATION[0], (sizeof(START_CALIBRATION) / sizeof(START_CALIBRATION[0]))};
    hid_->send_feature_report(0, (void *)setFeatureData.pData, setFeatureData.nDataLength);

    usleep(1000 * 1000 * 5);
  }

  // Check calbration status
  int timeout = 10;
  do {
    ...
    hid_->send_feature_report(0, (void *)setFeatureData.pData, setFeatureData.nDataLength);
    hid_->get_feature_report(0, (void *)&status[0], setFeatureData.nDataLength);
  ...
  // Check calbration success.
  {
    ...
    hid_->send_feature_report(0, (void *)setFeatureData.pData, setFeatureData.nDataLength);
    hid_->get_feature_report(0, &status[0], setFeatureData.nDataLength);

    if (1 == status[0]){
      LOGI("IMU Calbration Successed.");
    }else{
      LOGI("IMU Calbration Failed.");
    }
  }  
...
void print_imu_data(const ImuDataPacket& imu_data) {
  std::cout << std::dec;
  if (imu_data.flag == EYS3D_IMU_ACCEL) {
    std::cout << "    [accel] stamp: " << imu_data.timestamp
      ...
      << std::endl;
  } else if (imu_data.flag == EYS3D_IMU_GYRO) {
    std::cout << "    [gyro] stamp: " << imu_data.timestamp
      ...
      << ", temp: " << imu_data.temperature
      << std::endl;
  } else if (imu_data.flag == EYS3D_IMU_ACCEL_GYRO_CALIB) {
...
void detect_imu_data_stamp(const ImuDataPacket& imu_data) {
  ...
  if (stamp_diff <= 0) {
    // same or rollback
    std::cout << "[STAMP_WARN]["
        << (imu_data.flag == EYS3D_IMU_ACCEL ? "accel" : "gyro") << "]"
        << " stamp: " << stamp
...
#if 1
  imu_data.parsePacket_Quaternion(data);
#else
  imu_data.parsePacket(data, true);
#endif
  imu_data.timestamp = std::time(nullptr);
  imu.push_back(imu_data);
#if !defined(NDEBUG)
  LOGI("==============================\n");
  LOGI("TimeStamp [%d]\n", imu_data.timestamp);
  LOGI("Accel [%f, %f, %f]\n", imu_data._accelX, imu_data._accelY, imu_data._accelZ);
  LOGI("Gyro [%f, %f, %f]\n", imu_data._gyroScopeX, imu_data._gyroScopeY, imu_data._gyroScopeZ);
  LOGI("==============================\n");
#endif
#else
