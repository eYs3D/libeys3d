...
int hid_device::send_feature_report(int num, void *data, int len, int timeout)
{
  hid_t *hid = get_hid(num);
  ...
  return usb_control_msg(hid->usb,
                         USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_ENDPOINT_OUT,
                         0x09 /*HID set_report*/,
                         (3 /*HID feature*/ << 8) | (static_cast<char *>(data))[0],
                         hid->iface,
                         static_cast<char *>(data), len,
                         timeout);
}

int hid_device::get_feature_report(int num, void *data, int len, int timeout)
{
  hid_t *hid = get_hid(num);
...
int hid_device::open(int max, int usage_page, int usage) {
  ...
  for (usb_bus_t *bus = usb_get_busses(); bus; bus = bus->next) {
    for (usb_device_t *dev = bus->devices; dev; dev = dev->next) {
      ...
      hid_device_id id = { dev->descriptor.idVendor,
                           dev->descriptor.idProduct };
      if (std::find(device_ids.begin(), device_ids.end(), id) == device_ids.end()) continue;
...
void hid_device::process_usb_dev(int max,
  ...
  for (int i = 0; i < dev->config->bNumInterfaces && iface; i++, iface++) {
    ...
    int len = usb_control_msg(handle, USB_ENDPOINT_IN | USB_RECIP_INTERFACE,
                              USB_REQ_GET_DESCRIPTOR, (USB_DT_HID << 8), i,
                              (char *)buf, 0x9, 1000);
#else
...
bool hid_device::find_device() {
  ...
  for (usb_bus_t *bus = usb_get_busses(); bus; bus = bus->next) {
    for (usb_device_t *dev = bus->devices; dev; dev = dev->next) {
      ...
      hid_device_id id = { dev->descriptor.idVendor, dev->descriptor.idProduct};
      if (std::find(device_ids.begin(), device_ids.end(), id) == device_ids.end())continue;
