...
#include "eys3d_depth/device/convertor.h"
...
Image::pointer Device::GetImageDepth() {
  ...
  if (!depth_image_buf_) {
    ImageFormat format;
    format = ImageFormat::DEPTH_RAW;
    depth_image_buf_ = ImageDepth::Create(format,
...
int Device::OpenDevice(const DeviceMode& dev_mode) {
  ...
  EtronDI_SetupBlock(handle_, &dev_sel_info_, true);
