...
#include "eys3d_depth/device/device.h"
...
void Device::SetInfraredDepthOnly(const OpenParams& params) {
  ...
  if (params.dev_mode != DeviceMode::DEVICE_ALL) {
    ...
    EtronDI_EnableInterleave(handle_, &dev_sel_info_, false);
            ...
            params.stream_mode == StreamMode::STREAM_640x400) &&
               params.framerate != 60)
  {
...
void Device::SetIRMaxValue(const std::uint16_t &value) {
  if (!IsInitDevice()) {
    LOGE("%s, %d:: Device is not initial.", __FILE__, __LINE__);
    return;
  }
  LOGI("-- Set Max Infrared Intensity : %d", value);
  if (value > 5 && value < 16) {
    EtronDI_SetIRMaxValue(handle_, &dev_sel_info_, value);
  }
}
...
void Device::SetInfraredIntensity(const std::uint16_t &value) {
  ...
  if (value != 0) {
    LOGI("-- Set Infrared Intensity : %d", value);
...
bool Device::Open(const OpenParams& params) {
  ...
  depth_data_type_ = (int)params.depth_data_type;
  ...
  if (ETronDI_OK == ret) {
    ...
    if (depth_device_opened_) {
      ...
      RegisterSettings::DM_Quality_Register_Setting(handle_, &dev_sel_info_);
...
bool Device::IsHWPP()
{
  ...
  int ret = EtronDI_GetHWRegister(handle_, &dev_sel_info_,
                                  0xf424,
                                  &nHWPP,
...
void Device::SetHWPP(bool bEnabled)
{
  unsigned short value = 0;

  value = 0;
  EtronDI_GetHWRegister(handle_, &dev_sel_info_,
                        0xf424, &value,
                        FG_Address_2Byte | FG_Value_1Byte);
  value &= 0x0F;
  value |= (bEnabled ? 0x40 : 0x70);
  EtronDI_SetHWRegister(handle_, &dev_sel_info_,
                        0xf424, value,
                        FG_Address_2Byte | FG_Value_1Byte);
}
...
std::shared_ptr<CameraCalibration> Device::GetCameraCalibration(
  ...
  switch (stream_mode) {
    ...
    case StreamMode::STREAM_2560x720:  // 720p, hd
      index = 0; break;
    default:
      index = (open_params_.depth_height > 550) ? 1 : 1;
      break;
        // throw new std::runtime_error("StreamMode is unknown");
  }

  return GetCameraCalibration(index);
}
...
bool Device::GetCameraCalibrationFile(const StreamMode& stream_mode,
  ...
  case StreamMode::STREAM_2560x720: // 720p, hd
    ...
    break;
  default:
    index = (open_params_.depth_height > 550) ? 1 : 0;
    break;
    // throw new std::runtime_error("StreamMode is unknown");
  }

  return GetCameraCalibrationFile(index, filename);
}
...
void Device::GetStreamIndex(const std::int32_t& dev_index,
  ...
  *depth_res_index = -1;
  ...
  get_stream_size(stream_mode, &color_width, &color_height);
  if (0 == color_width || 0 == color_height)
  {
    color_width = open_params_.color_width;
    color_height = open_params_.color_height;
  }
  ...
  int i = 0;
  while (i < 64) {
    if (stream_temp_info_ptr->nWidth == color_width &&
        stream_temp_info_ptr->nHeight == color_height &&
  ...
  if (*color_res_index == -1) {
    LOGE("Error: Color Mode width[%d] height[%d] format[%s] not support. "
        "Please check the resolution list.", color_width, color_height,
        get_stream_format_string(color_stream_format));
    *color_res_index = 0;
  }

  int depth_width = 0, depth_height = 0;
  get_stream_size(stream_mode, &depth_width, &depth_height);
  if (0 == depth_width || 0 == depth_height)
  {
    depth_width = open_params_.depth_width;
    depth_height = open_params_.depth_height;
  }

  stream_temp_info_ptr = stream_depth_info_ptr_;
  i = 0;
  while (i < 64) {
    if (stream_temp_info_ptr->nHeight == depth_height &&
  ...
  if (*depth_res_index == -1) {
    LOGE("Error: Depth Mode width[%d] height[%d] format[%s] not support. "
        "Please check the resolution list.", depth_width, depth_height,
...
#define EXTEND_LOW_FRAMERATE (1.2f)
void Device::Sleep()
{
  if (0 == GetOpenParams().low_framerate) return;

  double real_frequency = 1000.0 / GetOpenParams().framerate;
  double target_frequency = 1000.0 / (GetOpenParams().low_framerate * EXTEND_LOW_FRAMERATE);
  int64_t remain_frequecy = target_frequency - real_frequency;
  if (remain_frequecy > 0)
  {
    ++stream_count;
    if (GetOpenParams().dev_mode == DeviceMode::DEVICE_ALL && stream_count != 2)
      return;

    bool is_stream_enb = true;
    unsigned short temp1 = 0, temp2 = 0;

    if (is_stream_enb &&
        ETronDI_OK == EtronDI_GetHWRegister(handle_, &dev_sel_info_, 0xF05B, &temp1, FG_Address_2Byte | FG_Value_1Byte))
    {
      temp2 = temp1;
      temp1 &= 0xFC;

      if (ETronDI_OK == EtronDI_SetHWRegister(handle_, &dev_sel_info_, 0xF05B, temp1, FG_Address_2Byte | FG_Value_1Byte))
      {
        is_stream_enb = false;
      }
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(remain_frequecy));

    if (!is_stream_enb)
    {
      if (ETronDI_OK == EtronDI_SetHWRegister(handle_, &dev_sel_info_, 0xF05B, temp2, FG_Address_2Byte | FG_Value_1Byte))
      {
        is_stream_enb = true;
      }
    }
