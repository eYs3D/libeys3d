...
bool Thread::start() {
    ...
    if (pthread_create(&mThread, mStackSize ? &attr : nullptr, thread_main,
                       this)) {
        LOG_ERR(LOG_TAG, "Thread: failed to create a thread");
        ret = false;
...
bool Thread::tryWait(intptr_t* exitStatus) {
    ...
    if (!mJoined) {
        if (pthread_join(mThread, NULL)) {
            LOG_WARN(LOG_TAG, "Thread: failed to join a finished thread");
        }
        mJoined = true;
    }
...
void* Thread::thread_main(void* arg) {
...

    {
        ...
        if ((self->mFlags & ThreadFlags::Detach) != ThreadFlags::NoFlags) {
            if (pthread_detach(pthread_self())) {
                ...
                LOG_VERBOSE(LOG_TAG, "Failed to set thread to detach mode");
            }
        }
    ...
    libeYs3D::base::ThreadStoreBase::OnThreadExit();
