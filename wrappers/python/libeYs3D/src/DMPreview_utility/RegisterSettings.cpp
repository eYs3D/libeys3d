...
#define LOG_TAG "RegisterSettings"
...
int RegisterSettings::DM_Quality_Register_Setting_For15cm(void* hEtronDI, PDEVSELINFO pDevSelInfo)
{
    ...
    if (devinfo.wPID == ETronDI_PID_8038_M1) {
    ...
    sprintf(fileName, "%s/cfg/DM_Quality_Cfg/%s_DM_Quality_Register_Setting.cfg",
            libeYs3D::EYS3DSystem::getEYS3DSystem()->getSDKHomePath(), modelName);
    ...
    while (in) {
            ...
            if (RegValue != RegValueCheck) {
                printf("Set config. to Register failed: %s !!\n", tmp);
                return -1;
            }
        }
    }
...
int RegisterSettings::DM_Quality_Register_Setting_For6cm(void* hEtronDI, PDEVSELINFO pDevSelInfo)
{
    ...
    if (devinfo.wPID == ETronDI_PID_8038_M0) {
...
int RegisterSettings::DM_Quality_Register_Setting(void* hEtronDI, PDEVSELINFO pDevSelInfo, unsigned short wPID)
{
    ...
    sprintf(fileName, "%s/cfg/DM_Quality_Cfg/%s_DM_Quality_Register_Setting.cfg",
            libeYs3D::EYS3DSystem::getEYS3DSystem()->getSDKHomePath(), modelName);
    LOG_INFO(LOG_TAG, "Loading register settings: %s", fileName);
    ...
    while (in) {
            ...
            if (in) {
                ...
                { // ignore comments
                    const char *seps = "\t\n\v\f\r ";
                    { // ltrim
                        size_t totrim = strspn(tmp, seps);
                        if(totrim > 0)    {
                            size_t len = strlen(tmp);
                            if(totrim == len) {
                                tmp[0] = '\0';
                            } else    {
                                memmove(tmp, tmp + totrim, len + 1 - totrim);
                            }
                        }
                    }
                    { //rtrim
                        int i = strlen(tmp) - 1;
                        while(i >= 0 && strchr(seps, tmp[i]) != NULL)    {
                            tmp[i] = '\0';
                            i--;
                        }
                    }
                    
                    // ignore comments, empty line or starting with "#", "//"
                ...
                EtronDI_GetHWRegister(hEtronDI, pDevSelInfo, RegAddress, &RegValueCheck, FG_Address_2Byte | FG_Value_1Byte);
                if (RegValue != RegValueCheck) {
                    LOG_ERR(LOG_TAG, "Set config. to Register failed: %s ", tmp);
                    return -1;
                }
            }
    }
