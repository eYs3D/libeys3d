...
class CVDemoControl(Control):
    ...
    def execute(self):
        if self.mode_info.iInterLeaveModeFPS:
            cv_sample(self.camera_device, self.config,
                      True)  # Enable InterleaveMode
        else:
            cv_sample(self.camera_device,
                      self.config)  # Disable InterleaveMode
...
class CBDemoControl(Control):
    ...
    def execute(self):
        if self.config.get_config()['colorHeight'] is 0:
            print(Exception("This mode does not execute callback preview."))
        else:
